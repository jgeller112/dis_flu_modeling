...
# Experiment 2: Semantic Categorization Word Frequency and Disfluency
...
```{r}
p1_wf<-conditional_effects(fit_sc, terms=c("blur","freq"),  dpar = "mu")
p2_wf<-conditional_effects(fit_sc, terms=c("blur","freq"), dpar = "sigma")
p3_wf<-conditional_effects(fit_sc, terms=c("blur","freq"), dpar = "beta")


plot_mu_sc <- plot(p1_wf, plot = FALSE)[[3]] +  labs(x = "Blur", y = "Mu") + scale_x_discrete(labels=c('Clear', 'High Blur', 'Low Blur')) + theme_minimal(base_size=36)+ ggeasy::easy_all_text_size(size=36)  + 
  ggeasy::easy_move_legend("top")


plot_sig_sc <- plot(p2_wf, plot = FALSE)[[3]] +  labs(x = "Blur", y = "Sigma") + scale_x_discrete(labels=c('Clear', 'High Blur', 'Low Blur')) + theme_minimal(base_size=36)+ ggeasy::easy_all_text_size(size=36) + 
  ggeasy::easy_move_legend("top")


plot_beta_sc <- plot(p3_wf, plot = FALSE, size=5)[[3]] +  labs(x = "Blur", y = "Tau") + scale_x_discrete(labels=c('Clear', 'High Blur', 'Low Blur')) + theme_minimal(base_size=36)+ ggeasy::easy_all_text_size(size=36)  + 
  ggeasy::easy_move_legend("top")


plot_ex<-plot_mu_sc + plot_sig_sc + plot_beta_sc

plot_ex + ggeasy::easy_all_text_size(size=36)
...
scale_y_continuous(breaks=seq(0,100,10)) +
  ...
  geom_label_repel(data=v_chb_freq, aes(y=diff, label=round(diff,0)), color="black", min.segment.length = 0, seed = 42, box.padding = 0.5, size=10)+
  labs( title = "Delta Plots: Freq x Blur", x = "Mean RT per quantile", y = "Frequency Effect") + 
  ggeasy::easy_all_text_size(size=36)
...
ggsave("dprime_expt2.png", width=20, height=10, dpi=500)
